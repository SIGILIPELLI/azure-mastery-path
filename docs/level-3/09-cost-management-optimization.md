...
**Reserved Instances and Savings Plans** work by Azure matching your
committed spend against your actual usage at billing-reconciliation time,
not by provisioning anything different at the resource level — a Reserved
VM Instance is a billing construct scoped to a specific VM size/region (or,
with instance-size flexibility, a size-family) that the billing system
applies as a discount to any matching running VM's hourly usage record
retroactively each billing cycle, which is why buying a reservation doesn't
require touching or restarting any existing VM: the VM keeps running
exactly as before, and the discount is applied purely in the billing
pipeline based on matching meter IDs. A Savings Plan works the same way but
matches on committed hourly *spend* across a broader set of compute meters
rather than a specific VM size, which trades some discount depth for
flexibility across instance types.

**Azure Advisor's cost recommendations** are generated by a background
analysis job that queries the same Azure Monitor metrics pipeline from
Module 8 of Level 1 (average CPU/memory utilization over a trailing window,
typically 7–14 days) against each VM/disk's actual provisioned capacity,
flagging resources whose sustained utilization falls under a threshold as
candidates for right-sizing or deallocation — it's the identical metrics
store used for autoscaling and alerting, just queried by a cost-focused
analysis engine instead of a human dashboard. **Azure Cost Management**'s
budgets and cost-by-tag reports are built on top of the underlying usage
and billing records every resource provider emits per-meter, per-hour,
tagged with whatever resource tags were set at the time of that usage —
this is the exact mechanism behind why retroactively tagging a resource
does not retroactively re-tag its historical cost records: the tag
association is captured at usage-emission time, not read live from the
resource's current tag set.
